...
test_that("sqlite works", {
  ...
  if (dir.exists(test_dir)) {
    expect_s3_class(calls_sqlite <- etl("nyc311", dir = test_dir), "src_sqlite")
    expect_message(calls_sqlite %>% etl_init(), "Could not find")
    expect_message(calls_sqlite %>% etl_update(years = 2011, months = 1), "Writing NYC311 data")
    expect_output(print(calls_sqlite), "calls")
    expect_equal(calls_sqlite %>% tbl("calls") %>% collect(n = Inf) %>% nrow(), 170556)
  }
})
...
test_that("mysql works", {
  if (require(RMySQL) & mysqlHasDefault()) {
    ...
    if (dir.exists(test_dir)) {
      ...
      expect_message(calls_mysql %>% etl_update(years = 2011, months = 1), "Writing NYC311 data")
      expect_output(print(calls_mysql), "calls")
      expect_equal(calls_mysql %>% tbl("calls") %>% collect(n = Inf) %>% nrow(), 170556)
    }
  }
})
