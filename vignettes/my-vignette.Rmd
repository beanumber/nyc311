...
One obvious question is how many days it takes for NYC agencies to resolve each case. We can compute this by substracting `closed_date` from `created_date`.
...
For these calls, it took the agencies approximately 12 hours and 47 minutes to solve and close an issue.
...
agency_diff %>%
  head(10)
```
It seems that `New York City Police Department` is doing a great job on quickly solving problems. It takes the police on average less than 10 minutes to solve each issue.
...
The 2014 St. Patrick’s Day NYC Parade is one of the Top 10 largest parades that have ever happened in New York City, so let's take a look at the types of issue happened on March 17, 2014.

```{r}
calls2 <- etl("nyc311")
calls2 %>%
  etl_extract(years = 2014, months = 3 ) %>%
  etl_transform(years = 2014, months = 3) %>%
  etl_load(years = 2014, months = 3)

calls2 <- calls2 %>%
  tbl("calls") %>%
  collect()

nyc_2014_parade <- calls2 %>%
  filter(city == "NEW YORK") %>%
  group_by(complaint_type) %>%
  summarise(N = n()) %>%
  arrange(desc(N))

head(nyc_2014_parade,5)
```

The 2014 St. Patrick’s Day NYC Parade took place in New York City, so I first filtered all the issues that were reported in New York. As shown in the table above, `Noise - Residential` is the most common `complaint_type` on that day. This makes sense, since the parade was loud and has lasted for a along time. 
