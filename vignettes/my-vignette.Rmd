...
The 2014 St. Patrick’s Day NYC Parade is one of the Top 10 largest parades that have ever happened in New York City, so let's take a look at the locations of the first 100 issues happened on March 17, 2014.

```{r}
calls2 <- etl("nyc311")
calls2 %>%
  etl_extract(years = 2014, months = 3, num_calls = 100 ) %>%
...
if (require(leaflet)) { 
  leaflet(data = St.Patrick) %>% 
    addTiles() %>%
    addCircles()
}
```

As shown in the map above, most of the first 100 calls reported issues happened in New York City. This makes sense, since the 2014 St. Patrick’s Day NYC Parade took place in Manhattan (`NEW YORK` country). 
